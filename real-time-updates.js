...
window.stopRealtimeUpdates = stopRealtimeUpdates;
    // リアルタイム更新機能 - Server-Sent Events (SSE) 実装
...
class RealtimeUpdates {
    ...
    connect() {
        ...
        console.log('[リアルタイム更新] サーバーに接続中...');
        
        try {
            this.eventSource = new EventSource(`${API_BASE_URL}/events`);
            
            this.eventSource.onopen = () => {
                console.log('[リアルタイム更新] サーバーに接続されました');
                this.isConnected = true;
                this.reconnectAttempts = 0;
                this.reconnectInterval = 3000;
                this.showConnectionStatus('connected');
            ...
            this.eventSource.onmessage = (event) => {
                try {
                    ...
                    console.error('[リアルタイム更新] メッセージ解析エラー:', error);
                }
            };
            
            this.eventSource.onerror = (error) => {
                console.warn('[リアルタイム更新] 接続エラー:', error);
                this.isConnected = false;
                this.showConnectionStatus('error');
            ...
            console.error('[リアルタイム更新] 接続失敗:', error);
            this.attemptReconnect();
        }
    }
    ...
    attemptReconnect() {
        ...
        console.log(`[リアルタイム更新] ${delay/1000}秒後に再接続します (試行 ${this.reconnectAttempts}/${this.maxReconnectAttempts})`);
        this.showConnectionStatus('reconnecting', this.reconnectAttempts);
    ...
    handleServerMessage(data) {
        console.log('[リアルタイム更新] サーバーメッセージ受信:', data.type, data);
        
        switch (data.type) {
            case 'connected':
                console.log('[リアルタイム更新] サーバー接続確認');
                break;
            ...
            case 'reservation_added':
                this.handleReservationAdded(data.data);
                break;
            ...
            case 'break_mode_updated':
                this.handleBreakModeUpdated(data.data);
                break;
                
            case 'population_updated':
                this.handlePopulationUpdated(data.data);
                break;
                
            default:
                console.log('[リアルタイム更新] 未知のメッセージタイプ:', data.type);
        }
    }
    
    handleReservationAdded(data) {
        console.log('[リアルタイム更新] 新しい予約が追加されました:', data.reservation);
        
        // 予約データを更新
        if (typeof reservations !== 'undefined' && Array.isArray(reservations)) {
            // 重複チェック
            const existingIndex = reservations.findIndex(r => r.reservationNumber === data.reservation.reservationNumber);
            if (existingIndex === -1) {
                reservations.push({
                    ...data.reservation,
                    id: data.reservation.reservationNumber
                });
                
                // UI更新
                this.updateUI('reservation_added');
                this.showNotification(data.message || '新しい予約が追加されました', 'success');
            }
        }
    }
    
    handleReservationUpdated(data) {
        console.log('[リアルタイム更新] 予約が更新されました:', data.reservation);
        ...
        if (typeof reservations !== 'undefined' && Array.isArray(reservations)) {
            ...
            if (index !== -1) {
                ...
                // UI更新
                this.updateUI('reservation_updated');
                this.showNotification(data.message || '予約が更新されました', 'info');
            }
        }
    }
    
    handleBreakModeUpdated(data) {
        console.log('[リアルタイム更新] 休憩モードが更新されました:', data);
        ...
        if (typeof breakMode !== 'undefined') {
            ...
            this.showNotification(data.message || '休憩モードが更新されました', 'info');
        }
    }
    
    handlePopulationUpdated(data) {
        console.log('[リアルタイム更新] 待ち人数が更新されました:', data.count);
        ...
        // 軽い更新なので通知は控えめに
        this.showNotification(`待ち人数: ${data.count}人`, 'info', 2000);
    }
    ...
    showConnectionStatus(status, attempts = 0) {
        ...
        switch (status) {
            ...
            case 'reconnecting':
                message = `再接続中... (${attempts}/${this.maxReconnectAttempts})`;
                backgroundColor = '#ffc107';
                break;
                
            case 'error':
                message = 'リアルタイム更新: 接続エラー';
                backgroundColor = '#dc3545';
                break;
                
            case 'failed':
                message = 'リアルタイム更新: 接続失敗';
                backgroundColor = '#6c757d';
                break;
